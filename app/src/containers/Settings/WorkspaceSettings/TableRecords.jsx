...
import { TablePagination } from 'components-old/Paginations';
import portal from 'lib/portal';
import i18n from 'lib/i18n';
import { MODAL_UPDATE_RECORD } from './constants';
            ...
            render: (value, row, index) => {
              return (
                <FlexContainer fluid gutterWidth={0}>
                  <Row>
                    <Col width='auto'>
                      <div>{`Type = ${_get(row, 'controller.controllerType')}`}</div>
                      <div>{`Port = ${_get(row, 'controller.port')}`}</div>
                      <div>{`Baud Rate = ${_get(row, 'controller.baudRate')}`}</div>
                    </Col>
                  </Row>
                </FlexContainer>
              );
