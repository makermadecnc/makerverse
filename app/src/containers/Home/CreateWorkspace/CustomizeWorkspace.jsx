...
import ColorPicker from 'components-old/ColorPicker';
...
class CustomizeWorkspace extends React.PureComponent {
  ...
  renderIcon(icon) {
    return <img alt={`${icon.label} icon`} src={`/images/icons/${icon.value}.svg`} />;
  }
  ...
  render() {
    ...
    return (
      <div style={{ margin: 0, padding: 0 }}>
        ...
        {show && (
          <div className={cx('container-fluid', styles.widgetCenter)} style={{ padding: 20 }}>
            <div className='row'>
              <div className='form-group col-lg-6'>
                ...
                <div className={styles.widgetControl}>
                  ...
                  <br />
                  <ColorPicker color={bkColor} setColor={(c) => this.setSetting('bkColor', c.hex)} />
                </div>
                ...
                <div style={{ marginTop: 80 }}>
                  {canCreate && (
                    <button
                      type='button'
                      style={{ padding: 20 }}
                      className={cx('btn', 'btn-large', 'btn-primary')}
                      onClick={() => {
                        this.createWorkspace(workspaceSettings);
                      }}
                      disabled={this.props.creating}>
                      <h6 style={{ color: 'white' }}>{i18n._('Create Workspace')}</h6>
                      {i18n._("Your machine's new home.")}
                    </button>
                  )}
                  {this.props.createError && (
                    <ToastNotification type='error'>{this.props.createError}</ToastNotification>
                  )}
                </div>
              </div>
            </div>
        ...
        {!show && (
          <div className={styles.widgetCenter} style={{ fontStyle: 'italic', padding: 20, textAlign: 'center' }}>
