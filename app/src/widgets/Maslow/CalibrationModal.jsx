...
import MaslowCalibration, { sleds, selectedSled } from 'lib/Maslow/MaslowCalibration';
import styles from './index.styl';
import MeasureChainsFlow from './MeasureChainsFlow';
import { MODAL_CALIBRATION } from './constants';
...
class CalibrationModal extends PureComponent {
  ...
  getCalibrationRecommendation(result, accuracy) {
    ...
    if (result.change.avgErrDist > 0 || result.change.maxErrDist > 0) {
      ...
      return (
        pre +
        'Results are decent for edge calibration. You should strive for less than 6mm accuracy (lower is better). You can apply results and use Edge calibration again, or move on to the "Precision" tab if results do not improve with repeated calibration.'
      );
    } else if (accuracy <= 25) {
      return (
        pre +
        'For better accuracy, it is recommended that you run calibration again. You should strive for less than 6mm accuracy (lower is better). If this does not improve with repeated calibration, review the help.'
      );
    } else {
      return (
        pre +
        'Your machine is quite un-calibrated. Please review the help, and check that your frame build is not compromising accuracy.'
      );
    }
  }
  ...
  renderAlreadyCalibrated() {
    return (
      <div className={styles.bottom}>
        ...
        <Button btnSize='medium' btnStyle='flat' onClick={(event) => this.resetCalibration()}>
          ...
          <div className={styles.navContent} style={{ height: height }}>
            {curTab === 'welcome' && (
              <div className={styles.tabFull}>
                ...
                <br />
                <br />
                <Button btnSize='medium' btnStyle='primary' onClick={() => this.setState({ activeTab: 'machine' })}>
            ...
            {curTab === 'export' && (
              <div className={styles.tabFull}>
                ...
                {imported && <div className={styles.bottom}>Settings imported!</div>}
                <div className={styles.nextStep}>
                  <Button btnSize='medium' btnStyle='flat' onClick={(event) => this.import(exported)}>
                    <i className='fa fa-check' />
                    {i18n._('Import')}
                  </Button>
                </div>
            ...
            {curTab === 'machine' && (
              <div className={styles.tabFull}>
                <div className={styles.center} style={this.getBkImageStyle('calibration_overview.png')}>
                  <div style={{ position: 'absolute', bottom: '0px', right: '30%' }}>(Newtons)</div>
                ...
                <div className={styles.bottom}>
                  {!setMachineSettings && (
                    <div>These values on this tab should only be changed if you have a nonstandard setup.</div>
                  )}
                  {!setMachineSettings && (
                  ...
                  {!setMachineSettings && (
                    <div className={styles.nextStep}>
                      <Button btnSize='medium' btnStyle='primary' onClick={(event) => this.setMachineSettings()}>
                        <i className='fa fa-check' />
                        {i18n._('Apply')}
                      </Button>
                    </div>
            ...
            {curTab === 'stock' && (
              <div className={styles.tabFull}>
                ...
                <div className={styles.bottom}>
                  <div>Note: if your sled is not yet attached, it will be done later in Calibration.</div>
                  <div>
                    Stock Width:
                    <input
                      type='text'
                  ...
                  {nonstandardSize && (
                    <div>Note: you have entered a size different than a standard Maslow (4x8 feet).</div>
                  )}
                  {!setWorkspaceSettings && (
                    <div className={styles.nextStep}>
                      <Button btnSize='medium' btnStyle='primary' onClick={(event) => this.setWorkspaceSettings()}>
                        <i className='fa fa-check' />
                        {i18n._('Apply')}
                      </Button>
                    </div>
                  ...
                  {setWorkspaceSettings && (
                    <div className={styles.nextStep}>Now proceed to the &quot;Frame&quot; tab.</div>
                  )}
                </div>
              </div>
            )}
            {curTab === 'frame' && (
              <div className={styles.tabFull}>
                ...
                <div className={styles.top}>
                  ...
                  {!chainError && (
                    <div>
                      Enter approximate measurements, within 6mm (1/4&quot;) tolerance. Then, press &quot;Set
                      Frame&quot;.
                    </div>
                  )}
                </div>
                ...
                {!alreadyStartedCalibration && (
                  <div className={styles.bottom}>
                    ...
                    {setFrameSettings && <span className={styles.nextStep}>Now, move on to the next tab.</span>}
                  </div>
                )}
              </div>
            )}
                  ...
                  <div style={{ paddingTop: '20px' }}>
                    <span>
                      ...
                      <select
                        ...
                        <option value='' key='sled_custom_other'>
                          {i18n._('Other')}
                        </option>
                      </select>
                    </span>
            ...
            {curTab === 'z' && (
              <div className={styles.tabFull}>
                <div className={styles.top}>
                  ...
                  <br />
                  It may be skipped if you have already tested the Z-axis, or your machine came preconfigured for
                  Z-movement.
                </div>
                <div className={styles.center}>
                  {stepDirectionInvert && (
                  ...
                  {!stepDirectionInvert && !zAxisDistancePerRotation && (
                    <div>Your machine does not support Z-axis inversion.</div>
                  )}
                  {!stepDirectionInvert && zAxisDistancePerRotation && (
                    <div>If your Z-axis is inverted, press the Edit Raw Values button and use a negative number.</div>
                  )}
                  {zAxisResSetting && !zEditRaw && (
                    <div>
                      ...
                      />
                      <button type='button' className='btn btn-medium' onClick={() => this.zMove(Number(zMove))}>
                        Move Up
                      </button>
                      <button type='button' className='btn btn-medium' onClick={() => this.zMoveTo(0)}>
                        Go To Zero
                      </button>
                      <button type='button' className='btn btn-medium' onClick={() => this.zMove(-Number(zMove))}>
                        Move Down
                      ...
                      />
                      <button type='button' className='btn btn-medium btn-primary' onClick={() => this.zApplyScaling()}>
                        Apply Scaling
                      </button>
                      <br />
                      <br />
                      ...
                      <br />
                      <button type='button' className='btn btn-medium' onClick={() => this.zDefineZero()}>
                        Define Zero
                      </button>
                    </div>
                  )}
                  {!zAxisResSetting && <div>Your machine cannot change the Z-axis speed.</div>}
                  {zEditRaw && (
                    <div>
                      <h3>Raw Settings</h3>
                      <br />
                  ...
                  {zAxisDistancePerRotation && (
                    <div>
                      <br />
                      <div>
                        Your Maslow uses this additional value.
                        <br />
            ...
            {curTab === 'chains' && (
              <div className={styles.tabFull}>
                <div className={styles.top}>
                  ...
                  <br />
                  This will also create a &quot;save point&quot; to restore calibration (if chains ever come off
                  sprockets).
                  <br />
                ...
                {!alreadyStartedCalibration && measuredChains && (
                  <div className={styles.center}>
                    <h4>Chains Set</h4>
                    <span>
                      Accuracy is currently at {Math.max(1, Math.round(measuredChains.optimized.maxErrDist * 10) / 10)}
                      mm
                      <br />
                      <br />
            ...
            {isCalibrationTab && (
              <div className={styles.tabFull}>
                ...
                {hasCalibrationResult && (
                  <div className={styles.center}>
                    <table style={{ position: 'absolute', left: '0px', bottom: '0px' }}>
                      <tbody>
                        <tr>
                          <td />
                          <td>Old</td>
                    ...
                    <div style={{ position: 'absolute', right: '0px', bottom: '0px', width: '40%' }}>
                      <h4>New Accuracy: {accuracy}mm</h4>
                      <hr style={{ marginTop: '10px', marginBottom: '10px' }} />
                      {this.getCalibrationRecommendation(result, accuracy)}
                      <hr style={{ marginTop: '10px', marginBottom: '10px' }} />
                      These results must be applied to take effect! After they are applied, you may run Edge or
                      Precision calibration again.
                      <br />
                      <br />
                      {canApplyCalibration && (
                        <Button btnSize='lg' btnStyle='primary' onClick={(event) => this.applyCalibrationResults()}>
                          <i className='fa fa-check' />
                ...
                <div className={styles.bottom}>
                  ...
                  {isEdgeTab && <div>If the edge of the sled goes outside the stock, use a negative value.</div>}
                  {isPrecisionTab && (
                    <span>
                      {'Cut depth: '}
                      <input
                ...
                <div className={styles.middle}>
                  {canCalibrate && (
                    <div>
                      <Button btnSize='medium' btnStyle='primary' onClick={(event) => this.calibrate()}>
                  ...
                  <br />
                  {!hasCalibrationResult && (
                    <Button btnSize='medium' btnStyle='flat' onClick={(event) => this.moveToCenter()}>
                      <i className='fa fa-bullseye' />
        ...
        <Modal.Footer>
          {wiping && (
            <span style={{ float: 'left' }}>
              <Button style={{ marginRight: '10px' }} onClick={() => this.setState({ wiping: false })}>
                {i18n._('Cancel')}
              </Button>
              <strong>{i18n._('Are you sure?')} </strong>
              {i18n._('This will reset all machine & calibration settings (eeprom).')}
              <Button style={{ marginLeft: '10px' }} onClick={() => this.wipe()}>
                {i18n._('Accept')}
              </Button>
            </span>
          )}
          {!wiping && (
            <span style={{ float: 'left' }}>
              {wiped && <span style={{ float: 'left' }}>Settings have been wiped.</span>}
              {!wiped && (
                <Button style={{ marginRight: '40px' }} onClick={() => this.setState({ wiping: true })}>
