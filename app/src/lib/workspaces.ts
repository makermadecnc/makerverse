...
import { MachineEventType } from '@openworkshop/open-controller/controller';
import MachineController, { IPortOpts, MachineCommandType } from '@openworkshop/open-controller/MachineController';
import api from 'api';
import events from 'events';
import log from 'js-logger';
import MachineSettings from 'lib/MachineSettings';
import WorkspaceAxis from 'lib/workspace-axis';
import _ from 'lodash';
import { IWorkflow } from '../../../../OpenWorkEngine/npm-packages/open-controller/src/MachineController';
...
type WorkspaceRecord = IWorkspaceRecord;

/*
 * Each "Workspace" is a tab in the UI.
 */
class Workspaces {
  static all: { [key: string]: Workspace } = {};

  static findByPath(path: string): Workspace | undefined {
    return _.find(Workspaces.all, (w) => {
      return w.path === path;
    });
  }

  static load(record: WorkspaceRecord): Workspace | undefined {
    ...
    if (_.has(Workspaces.all, id)) {
      ...
      Workspaces.all[id] = new Workspace(record);
    }
    return Workspaces.all[id];
  }

  static async unload(id: string): Promise<void> {
    if (!_.has(Workspaces.all, id)) {
      return;
    }
  ...
  static disconnect(): void {
    Object.keys(Workspaces.all).map((id) => {
      const workspace: Workspace = Workspaces.all[id];
      void workspace.controller.disconnect();
    });
  }
}

export class Workspace extends events.EventEmitter {
  ...
  get workflow(): IWorkflow {
    return this.controller.workflow as IWorkflow;
  }
